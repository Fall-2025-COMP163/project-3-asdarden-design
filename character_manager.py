...
def revive_character(character):
    ...
    restored = character["max_health"] // 2
    if restored < 1:
        restored = 1
...
def validate_character_data(character):
    ...
    for k in numeric_fields:
        if not isinstance(character[k], int):
            raise InvalidSaveDataError(f"Invalid type for {k}: must be int")

    list_fields = ["inventory", "active_quests", "completed_quests"]
    for k in list_fields:
        if not isinstance(character[k], list):
            raise InvalidSaveDataError(f"Invalid type for {k}: must be list")
